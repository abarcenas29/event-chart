$(document).ready(function() {	
		...
		$.each(files, function(index, file) 
		{
			...
			fileReader.onload = (function(file) 
			{
				...
				{ 
					...
					dataArray.push({photo_name : file.name, file : this.result});
	...
	$('#upload-button .ec-poster-upload').click(function() 
	{
		...
		$('#loading-content').html('Uploading '+dataArray[0].photo_name);
		
		var uploadUrl = $('#drop-files').data('upload');
		var counter   = 1;
		$.each(dataArray, function(index, file) {
			$.post(uploadUrl, dataArray[index], function(data) 
			{
				var fileName = dataArray[index].file.photo_name;
				++x;
